...
const errorCallBack = <T>(
  errorRes: AxiosError<ApiResponse<T>>,
): ApiResponse<T> => {
...
export const AUTH_REQUEST = {
  ...
  get: <QT, ST>(endPoint: string, parameters?: QT): Promise<ApiResponse<ST>> =>
    axiosDefaultInstance
      .get(endPoint, { params: parameters })
      .then((res) => successCallBack<ST>(res))
      .catch((error) => {
        throw errorCallBack<ST>(error);
      }),
  post: <QT, ST>(
    endPoint: string,
    parameters?: QT,
    options?: object,
  ): Promise<ApiResponse<ST>> =>
    axiosDefaultInstance
      .post(endPoint, parameters, options)
      .then((res) => successCallBack<ST>(res))
      .catch((error) => {
        throw errorCallBack<ST>(error);
      }),
